...
- Name: logged-out-opportunities.spec.ts >> Logged-out user — opportunities >> sees "Learn more" on all cards (not Apply)
- Location: tests/logged-out-opportunities.spec.ts:34:7
...
Locator: locator('a[href^="/opportunities/"]').first().locator('text=Learn more')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('a[href^="/opportunities/"]').first().locator('text=Learn more')
