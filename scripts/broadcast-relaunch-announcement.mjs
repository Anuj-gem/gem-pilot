#!/usr/bin/env node
/**
 * Broadcast: relaunch announcement — "Stage 2: industry partners are
 * now in the app." Goes to every active customer (subscription_status
 * IN ('active','trialing')), excluding internal @gem.studio addresses.
 *
 * Subject + HTML + plain-text bodies are read from
 *   content/email-templates/relaunch_announcement.md
 * (the canonical template the rest of the launch references). Same
 * pattern + Postmark plumbing as scripts/broadcast-selznick-3-6.mjs.
 *
 * Modes:
 ...
 *
 * Dedupe: scripts/broadcast-relaunch-announcement-log.json. Re-running
 * --send is safe; previously-sent addresses are skipped.
 *
 * Run from gem-app/:
 *   node scripts/broadcast-relaunch-announcement.mjs           # dry run
 *   node scripts/broadcast-relaunch-announcement.mjs --test    # test send
 *   node scripts/broadcast-relaunch-announcement.mjs --send    # live broadcast
 */
...
// ── Cohort: trial + active customers, excluding internal @gem.studio
...
console.log(`Cohort: ${cohort.length} unique recipients (active=${activeCount}, trialing=${trialingCount})`)
if (cohort.length === 0) {
...
const SEND_LOG = path.join(
  process.cwd(),
  'scripts',
  'broadcast-relaunch-announcement-log.json'
)
let log = {}
try {
...
for (const r of cohort) {
  ...
  // Small spacing between sends — Postmark accepts 100/s but we don't
  // need to push that hard for 200 recipients.
