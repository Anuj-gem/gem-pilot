#!/usr/bin/env node
/**
 * Broadcast: opportunities announcement — "Introducing Opportunities."
 * Goes to every active customer (subscription_status IN
 * ('active','trialing')), excluding internal @gem.studio addresses.
 *
 * Subject + HTML + plain-text bodies are read from
 *   content/email-templates/opportunities_announcement.md
 *
 * Modes:
 ...
 *
 * Dedupe: scripts/broadcast-opportunities-announcement-log.json.
 * Re-running --send is safe; previously-sent addresses are skipped.
 *
 * Run from gem-app/:
 *   node scripts/broadcast-opportunities-announcement.mjs           # dry run
 *   node scripts/broadcast-opportunities-announcement.mjs --test    # test send
 *   node scripts/broadcast-opportunities-announcement.mjs --send    # live broadcast
 */
...
const TAG = 'opportunities-launch'
...
const SEND_LOG = path.join(
  process.cwd(),
  'scripts',
  'broadcast-opportunities-announcement-log.json'
)
let log = {}
try {
