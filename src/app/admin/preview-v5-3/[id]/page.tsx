// v5.3 preview page — reads v5.3 evaluation JSON from script_evaluations_pending
// keyed by submission_id. The URL [id] is the live script_evaluations.id (eval_id),
// which is what rescore-all.mjs logs and what the admin knows how to copy from the
// dashboard. We look up the submission_id, then fetch the pending row.
//
// Layout is identical to /admin/preview-v5-1 so outputs stay directly comparable.
...
export default async function PreviewV53Page({ params }: PageProps) {
  const { id } = await params
  const sb = createServiceClient()

  // Step 1: resolve eval_id → submission_id (and pull title for the header)
  const { data: evalRow, error: evalErr } = await sb
    .from('script_evaluations')
    .select('id, submission_id, script_submissions ( title )')
    .eq('id', id)
    .maybeSingle()

  if (evalErr || !evalRow) {
    return <NotFound id={id} reason={`Eval not found for id: ${id}`} />
  }

  const submissionId = (evalRow as any).submission_id as string
  const title =
    ((evalRow as any).script_submissions?.title as string | undefined) ??
    'Untitled'

  // Step 2: fetch the pending row that the rescore wrote
  const { data: pending, error: pendErr } = await sb
    .from('script_evaluations_pending')
    .select('evaluation, weighted_score, tier, model, created_at')
  ...
  if (pendErr || !pending) {
    return (
      <NotFound
        id={id}
        reason="No pending evaluation found for this submission."
        hint={`Run: node scripts/rescore-all.mjs --preview --eval-ids=${id}`}
      />
    )
  }

  const e = pending.evaluation as V53Evaluation
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 pb-24">
        ...
        >
          ...
          <span className="text-[11px] text-[var(--gem-gray-400)]">
            {pending.model ?? '—'} · {pending.created_at ? new Date(pending.created_at as string).toLocaleString() : ''}
          </span>
        </div>
        ...
        {/* Logline hero — v5.3 relaxes the hard cap to 50. Flag >50 as over cap. */}
        {positioning_hook && (
          <div
          ...
          >
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <div
                ...
                style={{
                  color: wordCount > 50 ? '#dc2626' : 'var(--gem-gray-500)',
                  background: wordCount > 50 ? 'rgba(220,38,38,0.08)' : 'transparent',
                  border: wordCount > 50 ? '1px solid rgba(220,38,38,0.25)' : '1px solid transparent',
                }}
              >
                {wordCount} words {wordCount > 50 && '· over cap'}
              </div>
            </div>
        ...
        <PreviewTabs
          pitch={
            <>
              ...
              {/* Lead Characters — the thing we're testing in v5.3 */}
              {lead_characters && lead_characters.length > 0 && (
                <Section
                  label="Lead Characters"
                  subtitle={`${lead_characters.length} character${lead_characters.length === 1 ? '' : 's'} · the parts inside this script and why an actor would chase them.`}
                >
                  <div className="space-y-3">
...
function NotFound({ id, reason, hint }: { id: string; reason: string; hint?: string }) {
  return (
    <>
      <Nav />
      <div className="max-w-3xl mx-auto px-4 py-16">
        <h1 className="text-xl font-semibold text-[var(--gem-gray-50)] mb-4">No v5.3 preview yet</h1>
        <p className="text-sm text-[var(--gem-gray-300)] mb-4">{reason}</p>
...
// --- Building blocks (identical to preview-v5-1) ---
