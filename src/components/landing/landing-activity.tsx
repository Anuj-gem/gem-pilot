...
export function LandingActivity() {
  return (
    <section className="px-6 sm:px-10 pt-20 sm:pt-28 pb-10 sm:pb-14" style={{ background: 'transparent' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          {[
            ...
            >
              ...
              <span className="block text-[36px] font-extrabold leading-none mb-1" style={{ color: '#111827' }}>{stat.number}</span>
              <span className="block text-[13px]" style={{ color: '#78716c' }}>{stat.label}</span>
            </div>
          ))}
        </div>

        {/* Leaderboard preview — hidden until leaderboard is ready
        <div className="rounded-2xl p-6" style={{ background: '#fff', border: '1px solid rgba(255,255,255,0.08)' }}>
          ...
        </div>
        */}
      </div>
